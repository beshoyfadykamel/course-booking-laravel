...
@section('content')
    <div class="max-w-2xl mx-auto">
        <div class="mb-6">
            <h1 class="text-3xl font-bold text-gray-900">{{ __('messages.edit_user') }} - {{ $user->name }}</h1>
            <p class="mt-2 text-gray-600">{{ __('messages.edit_user_desc') }}</p>
        </div>
            ...
            <form action="{{ route('users.update', $user->id) }}" method="POST" class="space-y-6">
                ...
                <div>
                    ...
                    <x-text-input id="name" class="block mt-1 w-full @error('name') border-red-500 @enderror"
                        type="text" name="name" placeholder="{{ __('messages.enter_name') }}"
                ...
                <div>
                    ...
                    <x-text-input id="email" class="block mt-1 w-full @error('email') border-red-500 @enderror"
                        type="email" name="email" placeholder="{{ __('messages.enter_email') }}"
