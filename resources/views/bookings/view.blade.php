            ...
            <div class="px-6 py-4 border-b border-gray-200 flex flex-col sm:flex-row sm:items-center sm:justify-between">
                ...
                <div class="flex gap-2 mt-2 sm:mt-0">
                    @if ($booking->trashed())
                        ...
                        <form action="{{ route('bookings.delete-permanently', $booking->id) }}" method="POST" class="inline-flex" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')">
                            @csrf
                            @method('DELETE')
                    ...
                    @else
                        <a href="{{ route('bookings.edit', $booking->id) }}" class="inline-flex items-center px-3 py-1.5 bg-orange-500 text-white rounded-lg hover:bg-orange-600 transition text-sm">
                            <i class="fas fa-pencil-alt me-1"></i> {{ __('messages.edit') }}
                        </a>
                        <form action="{{ route('bookings.destroy', $booking->id) }}" method="POST" class="inline-flex" onsubmit="return confirm('{{ __('messages.confirm_delete') }}')">
                            @csrf
                            @method('DELETE')
                            <button type="submit" class="inline-flex items-center px-3 py-1.5 bg-red-600 text-white rounded-lg hover:bg-red-700 transition text-sm">
                ...
                <dl class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    ...
                    <div>
                        ...
                        <dd class="mt-1 text-sm">
                            <a href="{{ $booking->user ? route('users.show', $booking->user->id) : '#' }}" class="text-gray-900 hover:underline">
