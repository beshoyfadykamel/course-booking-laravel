...
@section('content')
    <div x-data="bookingForm()">
                ...
                <form action="{{ route('bookings.update', $booking->id) }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 mt-1">
                            <select id="course_id" name="course_id" @change="fillCourseDetails()"
                            ...
                            <button type="button" @click="openCourseModal()"
                    ...
                    <div>
                        ...
                        <div class="flex gap-2 mt-1">
                            <select id="student_id" name="student_id" @change="fillStudentDetails()"
        ...
        <!-- Student Modal -->
        <div x-show="showStudentModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center"
            style="display: none;">
                ...
                <div class="px-6 py-4 space-y-3 max-h-96 overflow-y-auto">
                    <div class="text-center mb-4">
                        <img id="studentImage" src=""
                            class="w-24 h-24 rounded-full mx-auto object-cover border-2 border-gray-300"
                            x-show="studentImage">
                    </div>
                    <div class="space-y-2 text-sm">
                        <div class="flex justify-between"><span class="font-medium">{{ __('messages.id') }}:</span><span
                                id="studentId">-</span></div>
                        <div class="flex justify-between"><span class="font-medium">{{ __('messages.name') }}:</span><span
                                id="studentName">-</span></div>
                        <div class="flex justify-between"><span class="font-medium">{{ __('messages.email') }}:</span><span
                                id="studentEmail">-</span></div>
                        <div class="flex justify-between"><span
                                class="font-medium">{{ __('messages.country') }}:</span><span id="studentCountry">-</span>
                        </div>
                    </div>
                </div>
                <div class="px-6 py-4 border-t border-gray-200 flex justify-end">
                    <button @click="showStudentModal = false"
                        class="px-4 py-2 bg-gray-300 rounded-lg">{{ __('messages.close') }}</button>
                </div>
            </div>
        </div>

        <!-- Course Modal -->
        <div x-show="showCourseModal" x-transition class="fixed inset-0 z-50 flex items-center justify-center"
            style="display: none;">
            ...
            <div class="relative bg-white rounded-lg shadow-xl max-w-md w-full m-4 z-10">
                ...
                <div class="px-6 py-4 space-y-3 max-h-96 overflow-y-auto">
                    <div class="space-y-2 text-sm">
                        ...
                        <div class="flex justify-between"><span
                                class="font-medium">{{ __('messages.name') }}:</span><span id="courseName">-</span></div>
                        <div class="flex justify-between"><span
                                class="font-medium">{{ __('messages.description') }}:</span><span
