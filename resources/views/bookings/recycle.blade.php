    ...
    <div class="bg-white rounded-lg shadow-sm border border-gray-200 overflow-hidden">
        <div class="px-6 py-4 border-b border-gray-200">
            <h2 class="text-lg font-semibold text-gray-900">
                {{ __('messages.deleted_bookings_table') }}
                <span id="bookings_count" class="ms-2 px-2 py-1 bg-red-100 text-red-700 rounded-full text-sm">({{ $bookings_count }})</span>
            </h2>
        </div>
            ...
            <table id="data_table" class="w-full">
                ...
                <tbody class="divide-y divide-gray-200">
                    @if (isset($bookings) && $bookings->count() > 0)
                        @include('bookings.partials.recycle_table', ['bookings' => $bookings, 'searchTerm' => ''])
                    @endif
                </tbody>
            </table>
        </div>
            ...
            function fetchResults(page = 1, pushState = true) {
                ...
                ajaxRequest = $.ajax({
                    ...
                    success: function(data) {
                        ...
                        if (data.count !== undefined) { $('#bookings_count').text('(' + data.count + ')'); }
